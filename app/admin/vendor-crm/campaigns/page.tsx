...
import { VendorCampaignDirectory } from "@/components/admin/crm/campaigns/VendorCampaignDirectory";
...
const CAMPAIGN_DIRECTORY_LOAD_ERROR =
  "The campaign directory could not be loaded. Please retry — if this persists, contact engineering.";
...
// Admin-only campaign directory. Server-guarded, server-paged. No client
// service-role access; all reads run in this server component.
export default async function VendorCampaignDirectoryPage({
  searchParams,
}: {
  ...
  try {
    result = await listVendorCampaigns(query);
  } catch (e) {
    logCampaignRouteFailure("vendor-crm/campaigns", e);
    error = CAMPAIGN_DIRECTORY_LOAD_ERROR;
  }
