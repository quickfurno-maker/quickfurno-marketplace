...
check("23 the 80.14A pending record is byte-identical", () => {
  ...
  eq(MANIFEST.pendingPostAnchorMigrations.length, 10, "the pending set is exactly ten");
  ...
  eq(MANIFEST.pendingPostAnchorMigrations[8].version, "20260921100000", "lead drafts are the ninth pending authority");
  eq(MANIFEST.pendingPostAnchorMigrations[9].version, "20260922120000", "False Ceiling is the tenth pending authority");
});
...
check("24 applied and reconciled records are unchanged", () => {
  ...
  // Current ledger: ten applied + five reconciled + seven staging-applied + ten pending = 32.
  ...
  eq(MANIFEST.appliedPostAnchorMigrations.length + MANIFEST.reconciledPostAnchorMigrations.length +
     ...
     32, "and the four sets add up to it");
});
