...
record("V07a the final twenty-two versions are in exact chronological order",
  same(migrationFiles.slice(-22),
...
record("V07 the local migration set is exactly 114",
...
record("G06 pending holds exactly eight pinned source-only authorities, four are staging-applied, and the five governed authorities are reconciled as APPLIED",
  ...
  manifest.pendingPostAnchorMigrations[7].version === "20260918120000" &&
...
record("G08 the anchor post-anchor count agrees at 27",
  manifest.appliedAnchor?.postAnchorMigrationCount === POST_ANCHOR_COUNT);
record("G09 G1 is pinned to 114 / 10 applied / 5 reconciled / 4 staging-applied / 8 pending, not loosened",
  /const MIGRATION_COUNT = 114;/.test(g1Source) &&
  g1Source.includes(`version: "${BRIDGE_VERSION}"`) &&
  g1Source.includes(`sha: "${BRIDGE_SHA}"`) &&
  g1Source.includes("pendingPins.length === 8") &&
  g1Source.includes("stagingAppliedPins.length === 4") &&
