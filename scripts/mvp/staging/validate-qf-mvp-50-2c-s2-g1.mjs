...
const POST_ANCHOR_PENDING = [
  ...
  // QF-AOS-V2 is SOURCE-PENDING until its own isolated staging gate.
  {
    ...
    sha: "df243b7885b0ec49173ecbbf65c12d6f7609e03266a21747cc0694bd4585135e",
    phase: "QF-AOS-V2",
  },
].map((m) => ({
  ...m,
...
function validateState(state) {
  ...
  check("exactly twenty-three local migrations are newer than the anchor", postAnchorLocal.length === 23, `actual=${postAnchorLocal.length}`);
  ...
  check("anchor records the same post-anchor count", manifest.appliedAnchor?.postAnchorMigrationCount === 23);
  ...
  check("the explicit PENDING post-anchor set holds exactly the six pinned entries",
    pendingPins !== null && pendingPins.length === POST_ANCHOR_PENDING.length && pendingPins.length === 6,
