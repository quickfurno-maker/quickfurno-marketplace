...
const LIVE_MIGRATION_COUNT = 118;
...
check("12-13 R0 grew the tree by exactly one, from 103 to 104; the live tree is 118", () => {
  eq(MIGRATIONS.length, LIVE_MIGRATION_COUNT, "the live tree is 118");
...
check("14 the G1 live pin is the truthful current count", () => {
  ...
  assert(/const MIGRATION_COUNT = 118;/.test(g1), "G1 pins the live tree at 118");
...
check("23 the 80.14A pending record is byte-identical", () => {
  ...
  eq(MANIFEST.stagingAppliedPostAnchorMigrations.length, 8, "the staging-applied set is exactly eight");
  const falseCeiling = MANIFEST.stagingAppliedPostAnchorMigrations.find((record) => record.version === "20260922120000");
  eq(falseCeiling?.operationalStatus, "APPLIED_TO_STAGING", "False Ceiling is staging-applied");
  eq(falseCeiling?.appliedToProduction, false, "False Ceiling remains absent from production");
});
...
check("24 applied and reconciled records are unchanged", () => {
  ...
  // Current ledger: ten applied + five reconciled + eight staging-applied + eight pending = 31.
  eq(MANIFEST.appliedAnchor.postAnchorMigrationCount, 31, "post-anchor count is thirty-one");
  eq((MANIFEST.stagingAppliedPostAnchorMigrations ?? []).length, 8, "eight staging-applied");
  ...
  eq(MANIFEST.appliedPostAnchorMigrations.length + MANIFEST.reconciledPostAnchorMigrations.length +
     MANIFEST.stagingAppliedPostAnchorMigrations.length + MANIFEST.pendingPostAnchorMigrations.length, 31, "and the four sets add up to it");
});
...
check("M8 mutant: leaving the migration count stale", () => {
  ...
  assert(/const MIGRATION_COUNT = 118;/.test(g1), "and replaced by the truthful one");
