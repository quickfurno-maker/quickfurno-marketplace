...
const LIVE_MIGRATION_COUNT = 112;
...
check("12-13 R0 grew the tree by exactly one, from 103 to 104; the live tree is 112", () => {
  eq(MIGRATIONS.length, LIVE_MIGRATION_COUNT, "the live tree is 112");
...
check("14 the G1 live pin is the truthful current count", () => {
  ...
  assert(/const MIGRATION_COUNT = 112;/.test(g1), "G1 pins the live tree at 112");
...
check("24 applied and reconciled records are unchanged", () => {
  ...
  // Current ledger: ten applied + five reconciled + three staging-applied + seven pending = 25.
  eq(MANIFEST.appliedAnchor.postAnchorMigrationCount, 25, "post-anchor count is twenty-five");
  eq((MANIFEST.stagingAppliedPostAnchorMigrations ?? []).length, 3, "three staging-applied");
  eq(MANIFEST.appliedPostAnchorMigrations.length + MANIFEST.reconciledPostAnchorMigrations.length +
     ...
     25, "and the four sets add up to it");
});
...
check("M8 mutant: leaving the migration count stale", () => {
  ...
  assert(/const MIGRATION_COUNT = 112;/.test(g1), "and replaced by the truthful one");
