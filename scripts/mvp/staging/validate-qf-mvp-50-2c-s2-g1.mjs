...
//                     exactly NINE post-anchor migrations, ALL APPLIED, in
//                     exact order at remote history 21 through 29. ZERO pending
//                     remain. The last three statuses are founder-acknowledged
//                     existing staging state with catalog/source parity and
...
const POST_ANCHOR_APPLIED = [
  ...
  {
    // The ONLY post-anchor migration applied by the phase that pinned it, and the
    // only one whose evidence is first-party rather than imported: an exact-one
    // dry-run naming this file alone (seeds [], roles []), a single push, and an
    // independent re-list proving remote history 29 -> 30 with zero divergence.
    ...
    marker: "QF_MVP_50_5_STAGING_MIGRATION_APPLIED_AND_VERIFIED",
    evidenceType: "FIRST_PARTY_EXACT_ONE_DRY_RUN_VERIFIED_STAGING_EXECUTION",
    remoteHistory: 30,
    appliedByThisPhase: true,
    phase: "QF-MVP-50.5",
  },
].map((m) => ({
  ...m,
...
// QF-MVP-50.5 passed its own staging deployment gate (a real exact-one dry-run, a
...
const POST_ANCHOR_RECONCILED = [
  ...
  {
    // QF-MVP-75.01 — the MatchCore binding-rank-order replacement of
    // public.qf_assign_lead_vendors_v2. SOURCE ONLY: it has NOT been applied to
    // staging, it carries no applied evidence and no remote history count, and
    // it still requires its own separate staging deployment gate.
    version: "20260815000000",
    name: "qf_mvp_75_01_matchcore_binding_rank_order",
    sha: "ea65d8803f3d2510357d19a7aa779e74efc112089e787cc8c9802a13a4666707",
    phase: "QF-MVP-75.01",
  },
  {
    // QF-MVP-75.02 - geo normalization + the PostGIS shortlist foundation: two
    // GENERATED geography columns, two GiST indexes and ONE read-only,
    // service_role-only discovery RPC. SOURCE ONLY: it has NOT been applied to
    // staging, it carries no applied evidence and no remote history count, and
    // it still requires its own separate staging deployment gate. It is the
    // first QuickFurno migration to install a database EXTENSION (postgis, into
    // the `extensions` schema), which makes that separate gate mandatory rather
    // than a formality.
    version: "20260816000000",
    name: "qf_mvp_75_02_geo_postgis_shortlist",
    sha: "aa24f9d9299528d5bab11e18a7a53fd7f9a7a89833f94f1d18190deda9f2b1cd",
    phase: "QF-MVP-75.02",
  },
  {
    // QF-MVP-80.03 - audit_logs forward repair. The defining migration
    // (20260621000006_superadmin_foundation.sql) was NEVER applied to
    // production, so every superadmin admin/vendor action has been discarded
    // silently. This creates ONLY public.audit_logs, its read index and its two
    // admin RLS policies -- it does not replay the eight dead tables that
    // accompany it in the historical file, and it backfills no history.
    // SOURCE ONLY: it has NOT been applied to staging, it carries no applied
    // evidence and no remote history count, and it still requires its own
    // separate staging deployment gate.
    version: "20260817000000",
    name: "qf_mvp_80_03_audit_logs_forward_repair",
    sha: "13a44b111423d9d4b4cfc5dc8cbf24a66b96eb647ef4e0533856984b70273db8",
    phase: "QF-MVP-80.03",
  },
].map((m) => ({
  ...m,
...
const POST_ANCHOR_PENDING = [
  ...
  // QF-MVP-50.7 adds the THIRD pending entry: the stale-business terminalization
  // authority (20260906000000). Source only — not applied to staging, not applied
  ...
  // QF-AOS-V2 is SOURCE-PENDING until its own isolated staging gate.
  {
    version: "20260912040000",
    name: "qf_aos_v2_intelligence",
    sha: "a89c583bc7e25a4eed26d1e2e6aba0beacbd6e6b85d67226b8916efe2c9fc7d3",
    phase: "QF-AOS-V2",
  },
  // QuickFurno final lead-generation scope boundary: quality lead delivery only.
  {
  ...
  // QF-MVP-60 service availability remains source-only. Read-only migration-history
  // checks on 2026-09-18 proved it absent from both staging and production.
  {
    version: "20260915120000",
    name: "qf_jarvis_service_availability",
    sha: "32e2480dab0126e797bde4aaf14a5f8d1cc844350870afe13af6fcfb8e67189c",
    phase: "QF-MVP-60-SERVICE-AVAILABILITY",
  ...
  // Jarvis OS operator command receipts are SOURCE-PENDING. The migration has not
  // been applied to staging or production and grants no authority by existing on disk.
  {
    version: "20260924183000",
    name: "jarvis_os_operator_command_receipts",
    sha: "c8c66b3f8a55974f6ce92c8fe400bfacdc6ece4e06a431c02b3bc0cbe4b3ac3b",
    phase: "JARVIS-OS-OPERATOR-COMMAND-BRIDGE",
  },
].map((m) => ({
  ...m,
  filename: `${m.version}_${m.name}.sql`,
