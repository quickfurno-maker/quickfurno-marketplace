...
check("23 the 80.14A pending record is byte-identical", () => {
  ...
  eq(MANIFEST.pendingPostAnchorMigrations[8].version, "20260924183000", "Jarvis OS command receipts are the ninth pending authority");
...
check("24 applied and reconciled records are unchanged", () => {
  ...
  // Current ledger: ten applied + five reconciled + eight staging-applied + nine pending = 32.
  eq(MANIFEST.appliedAnchor.postAnchorMigrationCount, 32, "post-anchor count is thirty-two");
  ...
  eq(MANIFEST.appliedPostAnchorMigrations.length + MANIFEST.reconciledPostAnchorMigrations.length +
     MANIFEST.stagingAppliedPostAnchorMigrations.length + MANIFEST.pendingPostAnchorMigrations.length, 32, "and the four sets add up to it");
});
...
check("M8 mutant: leaving the migration count stale", () => {
  ...
  assert(/const MIGRATION_COUNT = 119;/.test(g1), "and replaced by the truthful one");
