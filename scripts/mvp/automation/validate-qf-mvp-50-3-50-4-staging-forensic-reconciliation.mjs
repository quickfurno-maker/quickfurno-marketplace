...
function validateState(state) {
  ...
  check("the exact final four forensic migration filenames are frozen, followed only by the applied 50.5 migration, the five reconciled governed authorities, the staging-applied Realtime membership and the three pinned SOURCE-PENDING authorities",
    same(state.migrationFiles.slice(-15),
  ...
  check("the manifest pending set holds exactly the three pinned source-only authorities, one is staging-applied, and the five governed authorities are reconciled as APPLIED",
    pending !== null && pending.length === 4 &&
  ...
  check("the anchor post-anchor count equals the ten applied records plus five reconciled, one staging-applied and three pending authorities",
    manifest.appliedAnchor?.postAnchorMigrationCount === EXPECTED_APPLIED.length + 5 + 1 + 4);
