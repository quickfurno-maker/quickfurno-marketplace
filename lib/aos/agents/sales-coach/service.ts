...
import { salesCoachAgentConfig } from "./agent.config";
import type { SalesCoachInput, SalesCoachOutput } from "./schema";

// QF-AOS-SalesCoach — FUTURE INACTIVE placeholder service.
...
export async function runSalesCoach(input: SalesCoachInput = {}): Promise<FutureAgentResult> {
  const output: SalesCoachOutput = {
    status: "future_inactive",
    message: "QF-AOS-SalesCoach is a future inactive placeholder. No action was taken.",
  };

  return {
    agentName: salesCoachAgentConfig.name,
    status: "future_inactive",
    mode: "placeholder",
