...
import { calendarSyncAgentConfig } from "./agent.config";
import type { CalendarSyncInput, CalendarSyncOutput } from "./schema";

// QF-AOS-CalendarSync — FUTURE INACTIVE placeholder service.
...
export async function runCalendarSync(input: CalendarSyncInput = {}): Promise<FutureAgentResult> {
  const output: CalendarSyncOutput = {
    status: "future_inactive",
    message: "QF-AOS-CalendarSync is a future inactive placeholder. No action was taken.",
  };

  return {
    agentName: calendarSyncAgentConfig.name,
    status: "future_inactive",
    mode: "placeholder",
...
export default runCalendarSync;
