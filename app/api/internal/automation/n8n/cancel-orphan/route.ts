// ============================================================================
// QF-MVP-50.6 — signed orphan-cancellation request
//
// n8n asks Core to sweep ONE orphan automation job. Core selects the job, proves
// the business entity is gone and moves it to the existing terminal state
// `cancelled`. n8n chooses nothing at all — not even a family to forward to,
// because there is nothing to forward: every outcome of this route ends the
// orchestration.
//
// The request carries THREE identity fields. There is no jobId, entityId,
// entityType, reason, safeCode, status, force, batch or limit field in the schema
// at all — which is stronger than accepting and ignoring them, because it makes
// "the orchestrator cannot choose a job to cancel" a property a reader can verify
// from the parser rather than a claim in a comment.
//
// THIS ROUTE IS NOT `recover` AND NOT `reconcile`. It can only terminalize an
// already-queued orphan; it can never open an attempt, classify one, finalize one
// or dead-letter one. The exact path below is a canonical signing field, so a
// signature minted for any other route can never authenticate here.
//
...
export async function POST(request: Request) {
  ...
  const verified = verifyN8nToCoreRequest({
    rawBody,
    method: "POST",
    path: N8N_CANCEL_ORPHAN_ROUTE_PATH,
  ...
  const parsed = parseCancelOrphanRequestBody(rawBody);
  if (!parsed.ok) {
  ...
  try {
    const result = await cancelOrphanAutomationJobForN8nTransport({
    ...
    // exactly where it was — still queued, still an orphan, and still eligible for
    // the next sweep. Failing to cancel is always safe; the dangerous direction is
    // cancelling something that should have run, and that direction is guarded by
    // the RPC's own post-write re-proof rather than by this handler.
    return rejected(
