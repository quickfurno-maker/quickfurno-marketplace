...
import { vendorOnboardAgentConfig } from "./agent.config";
import type { VendorOnboardInput, VendorOnboardOutput } from "./schema";

// QF-AOS-VendorOnboard — FUTURE INACTIVE placeholder service.
...
export async function runVendorOnboard(input: VendorOnboardInput = {}): Promise<FutureAgentResult> {
  const output: VendorOnboardOutput = {
    status: "future_inactive",
    message: "QF-AOS-VendorOnboard is a future inactive placeholder. No action was taken.",
  };

  return {
    agentName: vendorOnboardAgentConfig.name,
    status: "future_inactive",
    mode: "placeholder",
...
export default runVendorOnboard;
