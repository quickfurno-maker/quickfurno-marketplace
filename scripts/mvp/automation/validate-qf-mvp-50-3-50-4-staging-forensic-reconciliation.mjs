...
const GEO_POSTGIS_SHORTLIST_FILENAME =
  "20260816000000_qf_mvp_75_02_geo_postgis_shortlist.sql";
// QF-MVP-80.03 — the audit_logs forward repair, newest SOURCE-PENDING authority.
const AUDIT_LOG_REPAIR_FILENAME =
...
function validateState(state) {
  ...
  check("the manifest pending set holds exactly nine pinned source-only authorities, eight are staging-applied, and the five governed authorities are reconciled as APPLIED",
    ...
    pending[8].version === "20260924183000" && pending[8].operationalStatus === "PENDING" &&
    Array.isArray(state.manifest.stagingAppliedPostAnchorMigrations) &&
    state.manifest.stagingAppliedPostAnchorMigrations.length === 8 &&
    ...
    state.manifest.stagingAppliedPostAnchorMigrations[2].appliedToStaging === true &&
    state.manifest.stagingAppliedPostAnchorMigrations[2].appliedToProduction === true &&
    ...
    state.manifest.stagingAppliedPostAnchorMigrations[5].version === "20260918180500" &&
    state.manifest.stagingAppliedPostAnchorMigrations[5].operationalStatus === "APPLIED_TO_STAGING" &&
    state.manifest.stagingAppliedPostAnchorMigrations[5].appliedToStaging === true &&
    state.manifest.stagingAppliedPostAnchorMigrations[5].appliedToProduction === true &&
    state.manifest.stagingAppliedPostAnchorMigrations[5].stagingRemoteHistoryCountAfterApply === 44 &&
    state.manifest.stagingAppliedPostAnchorMigrations[6].version === "20260919010000" &&
    state.manifest.stagingAppliedPostAnchorMigrations[6].operationalStatus === "APPLIED_TO_STAGING" &&
    state.manifest.stagingAppliedPostAnchorMigrations[6].appliedToStaging === true &&
    state.manifest.stagingAppliedPostAnchorMigrations[6].appliedToProduction === true &&
    state.manifest.stagingAppliedPostAnchorMigrations[6].productionRemoteHistoryCountAfterApply === 53 &&
    state.manifest.stagingAppliedPostAnchorMigrations[5].productionRemoteHistoryCountAfterApply === 52 &&
    state.manifest.stagingAppliedPostAnchorMigrations[7].version === "20260922120000" &&
    state.manifest.stagingAppliedPostAnchorMigrations[7].operationalStatus === "APPLIED_TO_STAGING" &&
    state.manifest.stagingAppliedPostAnchorMigrations[7].appliedToStaging === true &&
    state.manifest.stagingAppliedPostAnchorMigrations[7].stagingRemoteHistoryCountAfterApply === 48 &&
    state.manifest.stagingAppliedPostAnchorMigrations[7].appliedToProduction === false &&
  ...
  check("the anchor post-anchor count equals ten applied, five reconciled, eight staging-applied and nine pending authorities",
    manifest.appliedAnchor?.postAnchorMigrationCount === EXPECTED_APPLIED.length + 5 + 8 + 9);
