...
**Superseded by QF-MVP-40.10D and QF-MVP-40.10E:** Wave 0 `qf_consent_help_response_v3` was subsequently submitted and **approved by Meta as UTILITY** (40.10D), and the Wave 1 canary `qf_lead_received_v1` was then submitted and **approved by Meta as UTILITY** too (40.10E). Both are `APPROVED_UNMAPPED` and **held from creation** (`submit_now: false`). Approval proves the provider contract only — it grants no consent, mapping, activation or send authority. See [QF-MVP-40.10D](QF-MVP-40-10D-WAVE0-CLOSURE-AND-WAVE1-CANARY.md) and [QF-MVP-40.10E](QF-MVP-40-10E-WAVE1-CANARY-CLOSURE.md). No migration. No deployment.
...
Since QF-MVP-40.10D the packet follows a **closed state model** rather than a uniform draft claim. As of QF-MVP-40.10E the closed set is **two** templates — `consent_help_response` (wave 0) and `lead_received` (wave 1) — each `approved` / `APPROVED_UNMAPPED` / `submit_now: false`, while every **other** entry remains `draft` / `DRAFT_NOT_SUBMITTED`. The set is pinned, so an unexpected approval anywhere else still fails the gate, and admitting a third requires a deliberate edit.
...
**Current status (QF-MVP-40.10E): THE UTILITY CONTRACT IS PROVEN FOR BOTH A NO-VARIABLE AND A
VARIABLE-BEARING TEMPLATE — Wave 0 v3 and the Wave 1 canary lead_received were each created with
exactly one POST and reconciled read-only to APPROVED with returned_category UTILITY. Both are
APPROVED_UNMAPPED and HELD FROM CREATION. Wave 0 v2 remains quarantined, neither deleted nor
...
remains deferred to QF-MVP-70. Wave 3 remains owner-reviewable but unauthorised. In Wave 1, `lead_received`
is closed (approved, unmapped, held) and the remaining 13 stay unauthorised; QF-MVP-40.10E proposes
`client_lead_status_update`, `client_matching_update` and `lead_assignment_alert` as the next exact
Utility subset, which is a proposal only — submitting any of them still requires explicit owner
authorization.
