// ============================================================================
// QF-MVP-50.2E — signed client-execution request
//
// n8n asks Core to execute the client work for the exact attempt it already
...
import { parseExecuteClientRequestBody } from "@/lib/automation/clientExecutionContract";
import { N8N_EXECUTE_CLIENT_ROUTE_PATH } from "@/lib/automation/transportTypes";
import { executeClientAutomationForN8nTransport } from "@/services/automationClientExecutionService";
...
export async function POST(request: Request) {
  ...
  // for the claim or completion route can never authenticate here.
  ...
  const parsed = parseExecuteClientRequestBody(rawBody);
  if (!parsed.ok) {
  ...
  try {
    const result = await executeClientAutomationForN8nTransport({
