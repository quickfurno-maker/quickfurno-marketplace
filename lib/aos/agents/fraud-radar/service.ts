...
import { fraudRadarAgentConfig } from "./agent.config";
import type { FraudRadarInput, FraudRadarOutput } from "./schema";

// QF-AOS-FraudRadar — FUTURE INACTIVE placeholder service.
...
export async function runFraudRadar(input: FraudRadarInput = {}): Promise<FutureAgentResult> {
  const output: FraudRadarOutput = {
    status: "future_inactive",
    message: "QF-AOS-FraudRadar is a future inactive placeholder. No action was taken.",
  };

  return {
    agentName: fraudRadarAgentConfig.name,
    status: "future_inactive",
    mode: "placeholder",
