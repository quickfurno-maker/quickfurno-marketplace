...
check("12-13 R0 grew the tree by exactly one, from 103 to 104; the live tree is 111", () => {
  eq(MIGRATIONS.length, LIVE_MIGRATION_COUNT, "the live tree is 111");
...
check("14 the G1 live pin is the truthful current count", () => {
  ...
  assert(/const MIGRATION_COUNT = 111;/.test(g1), "G1 pins the live tree at 111");
...
check("24 applied and reconciled records are unchanged", () => {
  ...
  // Current ledger: ten applied + five reconciled + two staging-applied + seven pending = 24.
  eq(MANIFEST.appliedAnchor.postAnchorMigrationCount, 24, "post-anchor count is twenty-four");
  ...
  eq(MANIFEST.appliedPostAnchorMigrations.length + MANIFEST.reconciledPostAnchorMigrations.length +
     ...
     24, "and the four sets add up to it");
});
...
check("M8 mutant: leaving the migration count stale", () => {
  ...
  assert(/const MIGRATION_COUNT = 111;/.test(g1), "and replaced by the truthful one");
