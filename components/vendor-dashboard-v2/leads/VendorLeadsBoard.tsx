...
export function VendorLeadsBoard({ leads, vendorId }: { leads: VendorLeadView[]; vendorId: string }) {
  ...
  const summaryTiles = [
    { key: "total", label: "Client matches", value: summary.total, icon: "leads" as const },
    { key: "contact", label: "Contact available", value: summary.contactAvailable, icon: "phone" as const },
    { key: "reported", label: "Match reviews", value: summary.reported, icon: "alert" as const },
  ];
    ...
    <>
      ...
      <div className="qf-vendor-v2-leads-controls">
        <label className="qf-vendor-v2-leads-search">
          <span className="qf-vendor-v2-sr-only">Search client matches by client, service or area</span>
      ...
      <p className="qf-vendor-v2-leads-count" role="status">
        {visible.length === leads.length ? `${leads.length} ${leads.length === 1 ? "match" : "matches"}` : `${visible.length} of ${leads.length} matches`}
      </p>
      ...
      {visible.length === 0 ? (
        <div className="qf-vendor-v2-empty">
          ...
          <p className="qf-vendor-v2-empty-title">No client matches found</p>
          <p className="qf-vendor-v2-empty-message">Clear the search to see every client match available to you.</p>
          <button type="button" className="qf-vendor-v2-btn qf-vendor-v2-btn--quiet" onClick={() => setQuery("")}>Show all matches</button>
        </div>
      ) : (
