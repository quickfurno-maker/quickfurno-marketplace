...
function validateState(state) {
  ...
  check("migration count is exactly 112", state.migrationFiles.length === MIGRATION_COUNT);
  ...
  check("the exact final four forensic migration filenames are frozen, followed only by the later governed migrations through the final lead-generation scope lock",
    same(state.migrationFiles.slice(-20),
  ...
  check("the manifest pending set holds exactly seven pinned source-only authorities, three are staging-applied, and the five governed authorities are reconciled as APPLIED",
    ...
    state.manifest.stagingAppliedPostAnchorMigrations.length === 3 &&
    ...
    state.manifest.stagingAppliedPostAnchorMigrations[2].appliedToProduction === false &&
  ...
  check("the anchor post-anchor count equals ten applied, five reconciled, three staging-applied and seven pending authorities",
    manifest.appliedAnchor?.postAnchorMigrationCount === EXPECTED_APPLIED.length + 5 + 3 + 7);
