...
## 7. Retry-stable policy result pre-read (MANDATORY)

The result-event pre-read is **mandatory** for `DISTRIBUTION_POLICY_EVALUATE`
(Correction 1). The reader is a **required** dependency of the policy evaluation
executor, and the pre-read always runs **first** — before recommendation
resolution, route resolution, quality read, config load, policy evaluation, and
event publication. A missing reader fails loudly with
`DISTRIBUTION_POLICY_RESULT_EVENT_READER_REQUIRED` before any authoritative read.
(The shared distribution-port interface keeps the reader optional only for
historical non-policy tasks; the policy branch requires it explicitly.)

The task computes the deterministic per-task result idempotency key and looks up
any durable result event already stored for the workflow task:
...
- **Found** → validate exact scope (entity type/lead/workflow/causation), the
  ...
  payload contract:
  - `approval_required` → audit decision = `require_human_approval`;
  - `auto_authorized` → passes the strict Phase 4B-1 validator;
  - `manual_review.required` → audit decision = `manual_review` **and**
    `payload.reason === audit.policy_reason_code` (Correction 3). A non-empty but
    different reason fails loudly with
    `POLICY_RESULT_REPLAY_INTEGRITY_ERROR:MANUAL_REASON_MISMATCH`.

  Then **reuse** the existing event: do NOT reload config, re-read quality,
  re-resolve the recommendation, re-evaluate, or republish. Any mismatch fails
...
**Auto-authorization cross-binding (Correction 2).** For a policy
auto-authorization event, the resolver additionally re-loads the ORIGINAL immutable
`lead.matching.completed` event referenced by `recommendation_event_id` (via
`resolveLeadDistributionRecommendation`) and verifies the auto event matches the
authoritative matching snapshot exactly: same lead id, workflow instance id,
recommendation event id, recommended vendor count, and recommended vendor ids in
the exact same order — and that the authorized ids equal those authoritative ids.
An internally-consistent but forged / corrupted / manually-inserted auto event that
names a different or reordered vendor list is rejected with
`AUTO_AUTH_RECOMMENDATION_SNAPSHOT_MISMATCH`; a missing/mismatched original matching
event fails loudly. No assignment or credit mutation occurs after a binding failure.
This never reruns matching, reranks, appends, truncates, or replaces vendors. The
human-approval path is unchanged (it does not cross-bind).
