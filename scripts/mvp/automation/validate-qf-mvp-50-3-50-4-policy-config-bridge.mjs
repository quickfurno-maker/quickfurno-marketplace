...
record("V07a the final fourteen versions are in exact chronological order",
  same(migrationFiles.slice(-14),
    [BRIDGE_NAME, ...FROZEN.map(([f]) => f), RECOVERY_NAME, CANARY_AUTHORITY_NAME,
     ...
     ORPHAN_CANCELLATION_NAME, STALE_BUSINESS_NAME]));
record("V07 the local migration set is exactly 106",
...
record("G08 the anchor post-anchor count agrees at 19",
  manifest.appliedAnchor?.postAnchorMigrationCount === POST_ANCHOR_COUNT);
record("G09 G1 was re-pinned to 106 / 10 applied / 5 reconciled / 1 staging-applied / 3 pending, not loosened",
