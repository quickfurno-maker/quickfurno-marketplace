...
// wildcard and no version-greater-than allowance: an eighth post-anchor migration,
...
function validateState(state) {
  ...
  check("exactly seven local migrations are newer than the anchor", postAnchorLocal.length === 7, `actual=${postAnchorLocal.length}`);
  check("the seven post-anchor migrations appear in exact pinned order", same(postAnchorLocal.map((record) => record.version), POST_ANCHOR_ORDER));
  check("anchor records the same post-anchor count", manifest.appliedAnchor?.postAnchorMigrationCount === 7);
  ...
  check("exactly two PENDING post-anchor migrations are declared", pendingPins !== null && pendingPins.length === 2, `actual=${pendingPins?.length}`);
...
function runMutants(pristineState) {
  const cases = [
    ...
    ["a new pending entry silently added", (state) => { state.manifest.pendingPostAnchorMigrations.push({ version: "20260811000000", name: "eighth", path: "supabase/migrations/20260811000000_eighth.sql", sha256: "f".repeat(64), phase: "QF-MVP-50.2F", operationalStatus: "PENDING", remoteVersionStatus: "NOT_PROVEN_OFFLINE", requiresSeparateStagingDeploymentGate: true, appliedByThisPhase: false }); }],
    ["the pending list key deleted entirely instead of emptied", (state) => { delete state.manifest.pendingPostAnchorMigrations; }],
    ["an eighth applied post-anchor migration", (state) => { state.manifest.appliedPostAnchorMigrations.push(clone(state.manifest.appliedPostAnchorMigrations[2])); }],
    ["an eighth post-anchor migration on disk", (state) => { state.migrations.push({ filename: "20260811000000_eighth.sql", version: "20260811000000", name: "eighth", sha256: "c".repeat(64), malformed: false }); }],
