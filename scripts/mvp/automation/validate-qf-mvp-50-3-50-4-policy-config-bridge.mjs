...
const STALE_BUSINESS_NAME =
  "20260906000000_qf_mvp_50_7_automation_stale_business_cancellation.sql";
// QF-MVP-40: the closure-only canary quiesce authority, newest and SOURCE-PENDING.
const CANARY_QUIESCE_NAME =
  "20260910060000_qf_mvp_40_canary_quiesce_transition.sql";
// QF LAUNCH SECURITY CLOSEOUT.
const LAUNCH_SECURITY_CLOSEOUT_NAME =
  "20260911000000_qf_launch_security_closeout.sql";
// QF-MVP-40.14: the newest pending authority — the Meta transactional mapping seed
...
record("V07a the final twenty versions are in exact chronological order",
  same(migrationFiles.slice(-20),
...
record("V07 the local migration set is exactly 112",
...
record("G06 pending holds exactly seven pinned source-only authorities, three are staging-applied, and the five governed authorities are reconciled as APPLIED",
  ...
  manifest.stagingAppliedPostAnchorMigrations[2].operationalStatus === "APPLIED_TO_STAGING" &&
  manifest.stagingAppliedPostAnchorMigrations[2].appliedToProduction === false &&
...
record("G08 the anchor post-anchor count agrees at 25",
  manifest.appliedAnchor?.postAnchorMigrationCount === POST_ANCHOR_COUNT);
record("G09 G1 is pinned to 112 / 10 applied / 5 reconciled / 3 staging-applied / 7 pending, not loosened",
  /const MIGRATION_COUNT = 112;/.test(g1Source) &&
  g1Source.includes(`version: "${BRIDGE_VERSION}"`) &&
  g1Source.includes(`sha: "${BRIDGE_SHA}"`) &&
  g1Source.includes("pendingPins.length === 7") &&
  g1Source.includes("stagingAppliedPins.length === 3") &&
...
const mutants = [
  ...
  // QF-MVP-80.14A: the pending set is non-empty again, so the guard is restated as
  // what it always meant — a RECONCILED version may never reappear as pending.
  ["demoting reconciled 090/100/110 back to pending is impossible",
    () => !manifest.pendingPostAnchorMigrations.some((r) => PENDING_ORDER.includes(r.version)) &&
