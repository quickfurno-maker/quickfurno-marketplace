...
import { whatsappPilotAgentConfig } from "./agent.config";
import type { WhatsappPilotInput, WhatsappPilotOutput } from "./schema";

// QF-AOS-WhatsAppPilot — FUTURE INACTIVE placeholder service.
...
export async function runWhatsappPilot(input: WhatsappPilotInput = {}): Promise<FutureAgentResult> {
  const output: WhatsappPilotOutput = {
    status: "future_inactive",
    message: "QF-AOS-WhatsAppPilot is a future inactive placeholder. No action was taken.",
  };

  return {
    agentName: whatsappPilotAgentConfig.name,
    status: "future_inactive",
    mode: "placeholder",
...
export default runWhatsappPilot;
