...
export interface LeadDistributionPolicyEvaluationDeps {
  ...
  /** Optional; when present, enables retry-stable result-event pre-read/reuse. */
...
export async function executeLeadDistributionPolicyEvaluationTask(
  ...
  // PART 7 — retry-stable pre-read. If a durable result event already exists for
  // this workflow task, validate + reuse it WITHOUT reloading config, re-reading
  // quality, re-evaluating the policy, or republishing.
