...
**QF-MVP-20.3B1A staging application: `PARTIAL_APPLICATION_REQUIRES_REVIEW`** ([record](QF-MVP-20-3B1-STAGING-APPLICATION-RESULTS.md)). One `npx supabase db push --linked`, exit **1**. **A committed · A2 committed · B1 failed and rolled back completely.** Staging migration history now holds **three** truthful rows (baseline, A, A2); B1 is correctly absent. Staging is coherent, not partially initialised: 67 tables, still 40 public functions, zero canonical functions, zero rows anywhere, `auth.users` 0, providers inactive, 0 enforcement triggers, 6 legacy RPCs intact, 67 policies intact.

**Root cause — authoring defect in B1, not a schema or staging problem.** B1's own §7.5c verification guard runs a negative regex over `pg_get_functiondef()`, which returns the definition **including comments**; the body of `qf_assign_lead_vendors_v2` documents that it never reads `app_settings` or `vendor_packages`, so the guard matched its own prose and aborted a compliant function. The executable SQL contains no configuration or package read — `ASSIGNMENT_CREDIT_COST` is still the internal literal `1`. Both offline validators passed because they inspect comment-stripped file text, while the in-database guard inspects comment-retaining catalog output.

**Next step is QF-MVP-20.3B1R2 — correct B1 only.** A and A2 are committed and their files must **not** be edited (that would desynchronise local and remote history). B1 has never applied anywhere, so correcting it is safe. Then dry-run (must propose exactly one pending migration), apply B1 alone, run the phase verifier to 58 PASS / 0 FAIL, then read advisors. **No `migration repair`, no `db reset`, no manual application.**

**QF-MVP-20.3B1R2 correction: `COMPLETE`** ([record](QF-MVP-20-3B1R2-CORRECTION-RESULTS.md)). Offline only — no database access, no SQL executed. **B1 corrected in place at version `20260723000300`** (SHA256 `46ce7377…`, was `a4b5c378…`): the unsound §7.5c negative regex over `pg_get_functiondef()` was withdrawn, along with two sibling positive source probes that a comment alone could satisfy. All nine positive catalog guards are retained. **No business behaviour changed** — the correction touches only the self-verification block and the header.

**The phase verifier carried the same defect and was corrected** (`b66ec060…`, was `688ab439…`): six rows made lexical assertions over `pg_get_functiondef()`, and **two of them would have failed against a correctly applied B1**. Each is now a catalog fact, a data fact, or an explicitly labelled informational row naming where the real proof lives. It still returns 58 checks and remains SELECT-only.

**The offline validator is hardened** to **126** checks (`734b6a13…`) with an executable-SQL view and seven regression fixtures — comments, block comments and string literals naming forbidden objects must PASS; real executable references and a raw `pg_get_functiondef` negative-regex guard must FAIL; unterminated constructs fail closed. It now also pins A and A2 to their applied hashes so an applied migration can never be edited.

**Migrations A and A2 are APPLIED and IMMUTABLE** (`b6307094…` / `9d77f446…`), represented by truthful remote history rows. B1 is editable precisely because its transaction rolled back and its version is absent remotely. **No migration repair and no staging reset is required.** The next push must apply **B1 only**.

**QF-MVP-20.3B1A2: B1 APPLIED — verification `57 PASS / 1 FAIL`** ([record](QF-MVP-20-3B1A2-STAGING-APPLICATION-RESULTS.md)). One dry-run proposing exactly one migration, then one `db push --linked` at **exit 0**. Staging migration history now holds **four** truthful rows (baseline, A, A2, B1). The 20.3B1R2 correction is proven: B1's §7 block ran to completion, and verifier rows 607 and 613 — the two that would have failed uncorrected — both passed.

**Remaining blocker before `APPLIED_AND_VERIFIED_ON_STAGING`:** `R03_lineage_append_only_grants` expected 0 UPDATE/DELETE grants on `lead_assignment_events` and found **4**. Supabase platform default privileges (`pg_default_acl` grants `arwdDxtm` on new public tables to `postgres`, `anon`, `authenticated`, `service_role`) applied when **Migration A** created the table. A correctly revoked from `public`/`anon`/`authenticated` — those hold **nothing** — but its `grant select, insert … to service_role` only *added*, never revoking the default `UPDATE`/`DELETE`/`TRUNCATE` from `service_role` or `postgres`. Append-only on lineage is therefore currently enforced by **neither privileges nor a trigger** (B2's immutability trigger does not yet exist). Zero rows are at risk today and no untrusted role is exposed, but the declared invariant is unmet.

**This is an A-scope gap and A is applied and immutable.** It must be closed by a **new forward migration** (proposed **QF-MVP-20.3B1G**) containing only `REVOKE` statements at a version above `20260723000300` — never by editing A, never by `migration repair`, never by `db reset`. The validator should also be extended so any migration creating a `public` table must explicitly `REVOKE ALL … FROM service_role, postgres` before its narrow `GRANT`; with Supabase default privileges, "grant only what you need" is insufficient on its own.

**QF-MVP-20.3B1G authored and reviewed: `GENERATED_REVIEWED_NOT_APPLIED`** ([record](QF-MVP-20-3B1G-LINEAGE-GRANT-REPAIR.md)). Migration **G** `20260723000400` (`91544524…`) is forward-only and REVOKE-only, restoring the declared append-only posture on `lead_assignment_events`: PUBLIC/anon/authenticated lose everything, `service_role` keeps **SELECT + INSERT only** and loses `UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER, MAINTAIN`. Owner/postgres is deliberately untouched as documented break-glass. Verifier is now **62 checks** (`e1d9edb8…`, history expects baseline + A + A2 + B1 + G); validator is **165/165** (`e27d62d0…`).

**Line-by-line review found and corrected four defects** rather than trusting the green validator: an explicit `begin;`/`commit;` wrapper that would have broken CLI transaction atomicity; PUBLIC proofs built on `information_schema.role_table_grants`, which omits PUBLIC grants and so could never match; vacuous fixtures testing a helper never applied to the migration; and a false-passing premise caused by verifier sequence number 46 colliding with the production-count guard. All lineage privilege proofs now use effective `has_table_privilege()` checks.

**Migration G is NOT applied and is NOT in the apply workspace.** Next step is QF-MVP-20.3B1G-A: copy G in, require exactly one pending version, apply once, verify **62 PASS / 0 FAIL**, then advisors.
