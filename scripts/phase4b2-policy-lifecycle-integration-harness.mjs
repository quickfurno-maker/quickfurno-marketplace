...
function makePolicyDeps(opts = {}) {
  const calls = { published: [], eventLookups: [], configLoads: 0, qualityReads: 0, scoreCalls: 0, matching: 0, assign: 0 };
  ...
  return {
    ...
    deps: {
      ...
      distribution: {
        ...
        resultEventReader: opts.resultEventReader,
      },
    },
  };
}
...
async function realAutoAuthorizedEvent() {
  const repo = new InMemoryRepo();
  await runPolicyDurable(repo, { snapshot: autoGuardedSnapshot(), quality: QUALITY_APLUS, rec: { recommendedVendorIds: ABC }, taskId: "task_make_auto" });
  const stored = [...repo.byKey.values()].find((e) => e.event_type === E.DISTRIBUTION_AUTO_AUTHORIZED);
  return stored;
}
