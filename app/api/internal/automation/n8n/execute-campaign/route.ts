// ============================================================================
// QF-MVP-50.4 — signed campaign-execution request
//
// n8n asks Core to execute the campaign work for the exact attempt it already
...
import { N8N_EXECUTE_CAMPAIGN_ROUTE_PATH } from "@/lib/automation/transportTypes";
import { executeCampaignAutomationForN8nTransport } from "@/services/automationCampaignExecutionService";
...
export async function POST(request: Request) {
  ...
  // the service re-proves it as exactly `campaign_execution` from durable job truth.
  const verified = verifyN8nToCoreRequest({
    rawBody,
    method: "POST",
    path: N8N_EXECUTE_CAMPAIGN_ROUTE_PATH,
  ...
  try {
    const result = await executeCampaignAutomationForN8nTransport({
